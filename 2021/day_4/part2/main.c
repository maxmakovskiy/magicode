...
int main() {
  ...
  print_stack(boards[0], 5);
  
  uint32_t* winners[1000];
  for (size_t i = 0; i < 1000; i++) {
    winners[i] = 0;
  }
  uint32_t winners_counter = 0;
  ...
  for (size_t i = 0; i < n_rounds; i++) {
    for (size_t j = 0; j < n_boards; j++) {
      ...
      int pos = find_pos(boards[j], rounds[i]);
      if (pos != -1) {
        marks[j]->data[pos] = 1;
      }
...
//     for (size_t k = 0; k < n_boards; k++) {
//       if (winners[k] == 1) { continue; }
//       int pos = find_pos(boards[k], rounds[i]);
//       if (pos != -1) {
//         marks[k]->data[pos] = 1;
//       }

//       int round_res = check_rows_win(marks[k]) || check_cols_win(marks[k]);
...
//         if (winners_counter == n_boards) {
//           printf("Board num #%d is the winner, current number %d\n", k, rounds[i]);
//           uint32_t score = calc_score(boards[k], marks[k], rounds[i]);
//           printf("SCORE: %d\n", score);
//           winner_found = 1;
//           break;
//         }
